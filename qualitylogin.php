...
{
	echo "<script>window.location='qualityaccount.php';</script>";
}
$error='';
if(isset($_POST['submit']))
{
	$sql = "SELECT * FROM qualitymanger WHERE loginid='$_POST[loginid]' AND password='$_POST[password]' AND status='Active'";
	$query = mysqli_query($con,$sql);
	if(mysqli_num_rows($query) == 1)
	{
		$SelectLogin = mysqli_fetch_array($query);
		$_SESSION['QualityID']= $SelectLogin['QualityID'] ;
		echo "<script>window.location='qualityaccount.php';</script>";
	}
	else
	{
...
<title>Trex's Hospital QualityManger</title>
...
?></div>
    ...
    <div class="card">
        <h1 class="title"><span>Trex's Hospital</span>Login <span class="msg">Hello, QualityManger!</span></h1>
        <div class="col-md-12">
