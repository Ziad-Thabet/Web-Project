...
if(isset($_POST['submit']))
{
	...
	{
		$LastID =$_SESSION['patientid'];
	}
	else
	{
		$date = date("Y-m-d");
		$time = date("H:i:s");
		$sql ="INSERT INTO patient(patientname,admissiondate,admissiontime,city,address,mobileno,loginid,password,gender,dob,status) values('$_POST[patiente]','$date','$time','$_POST[city]','$_POST[address]','$_POST[mobileno]','$_POST[loginid]','$_POST[password]','$_POST[select6]','$_POST[dob]','Active')";
	...
	{
		$sql ="INSERT INTO appointment(appointmenttype,patientid,appointmentdate,appointmenttime,app_reason,status,departmentid,doctorid) values('ONLINE','$LastID','$_POST[appointmentdate]','$_POST[appointmenttime]','$_POST[app_reason]','Pending','$_POST[department]','$_POST[doct]')";
		if($query = mysqli_query($con,$sql))
		{
                                        ...
                                        <li class="col-sm-6">
                                            <label>
                                                <input placeholder="Date of birth" type="text" class="form-control"
                                                    ...
                                                    value="<?php echo $SelectPatient['dob']; ?>" <?php echo $readonly; ?>><i
                                            ...
                                            <label>
                                                ...
                                                <select name="department" class="selectpicker" id="department"
                                                    ...
                                                    <?php
                                                        ...
                                                        while($rsdept = mysqli_fetch_array($querydept))
                                                        {
                                            ...
                                            <label>
                                                <select name="doct" class="selectpicker" id="department"
                                                    ...
                                                    while($rsdept = mysqli_fetch_array($querydept))
                                                    {
                                                        ...
                                                        $rsdept = mysqli_fetch_array($querydepta);
...
function validateform() {
    ...
    else if (document.frmpatapp.address.value == "") {
        alert("address should not be empty..");
        document.frmpatapp.address.focus();
        return false;
    } else if (!document.frmpatapp.address.value.match(alphaspaceExp)) {
        alert("address name not valid..");
        document.frmpatapp.address.focus();
        return false;
    } 
